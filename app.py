...
# 🧠 뉴스 수집 및 요약 함수 (초4, 중1, 전문가 모드 통합본)
...
def fetch_and_summarize(query, mode):
    ...
    # 2. 모드별 페르소나 및 규칙 설정
if "초등" in mode:
        # 초4: 듣기 편한 리듬감 + 쉬운 비유 + 용어는 나중에
        content_rule = """
        - 대상: 초등학교 4학년 (11살). 
        - 핵심 규칙: 문장을 아주 짧게 끊어서 말해줘. (호흡이 길면 듣기 힘들어해)
        - 지식 전달 순서: 쉬운 설명과 비유를 먼저 하고, 마지막에 "이걸 어려운 말로 OO라고 한단다"라고 덧붙여줘.
        - 분량: 주제는 딱 2가지만! 욕심내지 말고 아주 쉽게.
        - 말투: '말이야' 같은 추임새 대신, 다정한 엄마가 동화책 읽어주듯 리듬감 있게.
        """
        ...
        content_rule = """
        - 핵심 뉴스 3가지 선정.
        - 지식 전달: 시사 용어(환율, 금리 등)를 그대로 쓰되, 한 문장으로 친절하게 풀이.
        - 논리 구성: 사건의 원인과 결과를 중학생이 이해할 수 있게 연결.
        - 말투: "~하단다", "~란다" (지적이고 다정한 엄마 말투)
        """
        start_msg = "엄마가 오늘 뉴스 들려줄게."
        end_msg = "오늘 하루도 즐겁게 보내자!"
        ...
        - 지식 전달: 전문 용어와 구체적인 수치(데이터)를 적극 활용.
        - 말투: "~입니다", "~바랍니다" (신뢰감 있는 여성 아나운서 말투)
    ...
    - 마무리: "{end_msg}" 로 끝낼 것.
    - '첫째', '둘째' 같은 번호는 금지하고 자연스럽게 문장을 이어줘.
    - 불필요한 추임새(말이야, 있지 등)는 자제하고 담백하게 전달해줘.
