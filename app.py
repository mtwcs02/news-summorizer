...
with st.sidebar:
    ...
    st.info("💡 속도 개선을 위해 음성은 버튼을 누를 때만 생성됩니다.")
...
try:
    ...
    # 429 에러 방지를 위해 모델 선언을 더 가볍게 합니다.
...
# 4. 실행 로직
if user_input:
    # 세션 상태를 써서 결과물을 저장해둡니다 (새로고침 방지)
    with st.spinner(f"'{user_input}' 뉴스를 분석 중입니다..."):
        try:
            # 뉴스 가져오기
            ...
            if items:
                ...
                prompt = f"너는 {role}야. 키워드 '{user_input}'에 대한 뉴스 {len(items)}개를 읽고 3가지 핵심을 풍성하게 요약해줘."
                
                # 1. 글자부터 먼저 보여주기 (여기서 속도가 빨라집니다!)
                ...
                # 2. 음성은 필요할 때만! (선택 버튼)
                st.write("---")
                if st.button("🎧 음성 브리핑 듣기 (생성 시작)"):
                    with st.spinner("목소리를 입히는 중... (약 5초 소요)"):
